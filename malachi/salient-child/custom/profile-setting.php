...
class ProfileSetting
{
    ...
    function send_query(){
        ...
        $msg = wp_mail(get_option('admin_email'), "Kainamo Question Query", $email_content, $headers, $attachments);
        // $msg = wp_mail('testingserver442@gmail.com', "Kainamo Query", $email_content, $headers, $attachments);
    ...
    function update_store_profile() {
        ...
        $required_fields = ['store_name', 'store_email'];
        foreach ($required_fields as $value) {
            if (empty($_POST[$value])) {
                $response['icon'] = "error";
                $response['title'] = "Error";
