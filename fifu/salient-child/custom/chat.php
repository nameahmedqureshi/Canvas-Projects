...
class Chat{
    ...
    function enqueue_custom_scripts() {
        wp_enqueue_script('custom-ajax-script', get_template_directory_uri() . '/js/custom.js', array('jquery'), '1.0', true);
    ...
    function get_users() {
        ...
        global $wpdb;
        $current_user_id = get_current_user_id(); // Current logged-in user

        $chat_data = $wpdb->get_results("SELECT sender_id, receiver_id FROM " . $wpdb->prefix . "custom_chat", ARRAY_A);

        $all_sender_ids = array_column($chat_data, 'sender_id'); // Extract all sender IDs
        $all_receiver_ids = array_column($chat_data, 'receiver_id');


        // var_dump($all_receiver_ids);
        // var_dump($all_sender_ids);
        // exit;
        ...
        if ( !empty( $get_users ) ) {
            foreach ( $get_users as $user ) { 
                ...
                $is_sender_matched = in_array($current_user_id, $all_sender_ids);
                $is_receiver_matched = in_array($user->ID, $all_receiver_ids);

                // var_dump($current_user_id);
                // var_dump($is_receiver_matched);


                if (!in_array('farmer', wp_get_current_user()->roles) && !$is_receiver_matched) {
                ...
                $profile_img = !empty($get_profile_image_url) ? $get_profile_image_url : get_stylesheet_directory_uri() .'/multivendor/assets/images/avatar.png';
         ...
         // Check if no users were added to the HTML
        if (empty($users_html)) {
            $users_html = '<p class="no-users">No users Found!</p>';
        }
