...
class EventClass {
    ...
    function add_event() {
        ...
        if (empty($_POST['pickup_date']) || empty($_POST['notes'])) {
            $response['icon'] = "error";
            $response['status'] = false;
        ...
        if($post_id){
            ...
            update_post_meta($post_id, 'apply_date', $_POST['pickup_date']);
            update_post_meta($post_id, 'delievery_date', date('Y-m-d', strtotime('+45 days', strtotime(date($_POST['pickup_date'])))) );
        }
        ...
        if (is_wp_error($post_id)) {
                ...
                $response['message'] = "Schedule pickup successfully!";
                $response['auto_redirect'] = true;
                $response['status'] = true;
                $response['redirect_url'] = site_url('schedule-a-pickup');
		}
    ...
    function register_event() {
        register_post_type('schedule-pickups',
            array(
                'labels'      => array(
                    'name'          => __('Schedule Pickups', 'textdomain'),
                    'singular_name' => __('Schedule Pickups', 'textdomain'),
                ),
                    'public'      => true,
                    'has_archive' => true,
